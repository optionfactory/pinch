...
fn draw_combined_logs(state: &DashboardState, frame: &mut Frame, area: Rect) {
...
fn draw_process_pane(state: &DashboardState, frame: &mut Frame, area: Rect, pane_id: usize) {
    ...
    let link_btn_str = if pane.config.link.is_some() { " [↗]" } else { "    " };
    ...
    let mut title_spans = vec![
        ...
        Span::styled(link_btn_str, Style::default().fg(COLOR_BORDER_ACTIVE)),
...
fn draw_process_grid(state: &DashboardState, frame: &mut Frame, _grid_area: Rect) {
...
fn draw_footer(state: &DashboardState, frame: &mut Frame, area: Rect) {
    ...
    let help_text = Paragraph::new(help_str).style(Style::default().bg(COLOR_FOOTER_BG).fg(COLOR_MUTED_TEXT));
    frame.render_widget(help_text, area);
}

pub fn draw(state: &DashboardState, frame: &mut Frame) {
