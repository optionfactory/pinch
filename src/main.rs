    ...
    match parsed.action {
        cli::CliAction::Config(cmd) => match cmd {
            cli::ConfigCommand::Show { format } => {
                raw_config.show_config(&parsed.vars, format)?;
                return Ok(());
            }
            cli::ConfigCommand::Var { name, format } => {
                raw_config.show_vars(&parsed.vars, name.as_deref(), format)?;
                return Ok(());
            }
        ...
        cli::CliAction::Process(cmd) => match cmd {
            ...
            cli::ProcessCommand::Show { title, format } => {
                raw_config.show_processes(&parsed.vars, title.as_deref(), format)?;
                return Ok(());
            }
            cli::ProcessCommand::Run { title, background } => {
                ...
                let mut cmd = proc.to_std_command()?;
        ...
        cli::CliAction::Net(cmd) => match cmd {
            cli::NetCommand::Ls => {
                ...
                raw_config.list_networks();
                return Ok(());
            }
            cli::NetCommand::Show { name, format } => {
                raw_config.show_networks(&parsed.vars, name.as_deref(), format)?;
                return Ok(());
            }
            cli::NetCommand::Create { name } => {
                raw_config.create_networks(&parsed.vars, name.as_deref())?;
                if let Some(n) = name {
